...
sk = - step_size*solve(hess, grads_.mean(axis=0))
x = x + sk
...
for k in range(iters):
    ...
    sk = - step_size/np.sqrt(k+1)*solve(hess, grad)
    x = x + sk
    grads = quad.dobjf(x, smp)
    yk = grads - grads_
    bay.update_curv_pairs(sk, yk)
...
np.save('sgd_bay_decr/sgd_bay_decr_1e6.npy', [opt_gap, update_iters, op_err_norm])
...
fig.tight_layout()
fig.savefig('sgd_bay_decr/sgd_bay_decr_1e6.pdf')