...
def svrg(dataset, data_size, n_features, reg_param, label_format,
         step_param, batch_size,
         epochs, clean_data=False, seed=0):
    print(f'Using SVRG to train the {dataset} dataset')
    ...
    print(f'Finished using SVRG to train the {dataset} dataset')
    print(f'Time spent: {runtimes[-1]:.2f}')

    np.save(f'{dataset}/svrg.npy', [opt_gap, runtimes])

    fig, ax = plt.subplots(figsize=(6, 6))
    ax.semilogy(opt_gap, 'o-', ms=3., label='SVRG')
    ax.grid()
    ax.legend()
    ax.set_xlabel('Epochs')
    ax.set_ylabel('Opt. gap')
    fig.tight_layout()
    fig.savefig(f'{dataset}/SVRG_opt_gap.pdf')
