...
class SARAH():
    ...
    def evaluate(self, x, k):
        if self.verbose:
            print(f'Evaluating SARAH')
        if self.estim == [] or not (k % self.m):
            print('SARAH restart')
            # self.estim = np.zeros(x.shape)
            # for idx, samp in enumerate(self.sample):
            #     self.estim = (self.estim*idx + self.func(x, samp))/(idx+1)
        ...
        #                   - self.func(self.x_, self.sample[sample_idx]))
