...
def sarah_bay(dataset, data_size, n_features, reg_param, label_format,
              step_param, batch_size, epochs, update_when, clean_data=False,
              seed=0, bay_params={}):
    print(f'Using SARAH-Bay to train the {dataset} dataset')
    ...
    bay = BayHess(n_dim=prob.n_features, strong_conv=prob.reg_param,
                  smooth=prob.almost_sure_smooth, log=f'{dataset}/sarah_bay.txt',
                  **bay_params)
    ...
    while not df.force_exit:
        ...
        if last_update + update_when <= epoch < epochs:
            bay.print(f'SARAH-Bay iteration: {k}')
    ...
    print(f'Finished using SARAH-Bay to train the {dataset} dataset')
    ...
    np.save(f'{dataset}/sarah_bay.npy', [opt_gap, update_iters, runtimes, err])

    fig, ax = plt.subplots(figsize=(6, 6))
    ax.semilogy(opt_gap, 'o-', ms=3., label='SARAH-Bay')
    ax.grid()
    ax.legend()
    ax.set_xlabel('Epochs')
    ax.set_ylabel('Opt. gap')
    fig.tight_layout()
    fig.savefig(f'{dataset}/sarah_bay_opt_gap.pdf')
